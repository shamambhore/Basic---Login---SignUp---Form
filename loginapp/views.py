...
def signup(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username = username):
            messages.error(request, 'Username already taken, enter another username')
        ...
        if len(username)>10:
            messages.error('username should be less than 10 characters')
            
        if password != cpassword :
            messages.error(request, 'Password not matching')
            
        if not  username.isalnum():
            messages.error('username should be alpha numeric')
            return redirect('home')
        ...
        #gmail coding
        subject = "Welcome to the Login Sytem app"
        message = "Welcome" + myuser.first_name + "!!! \n" + "Welcome to the login system app \n Please confirm your email address \n"
...
def signout(request):
    logout(request)
    messages.success(request, "Use has been logged out successfully")
    return redirect('home')
